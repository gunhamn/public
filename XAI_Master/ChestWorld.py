...
class ChestWorld(gym.Env):
    ...
    def __init__(self, render_mode=None, size=6, agentSpawn = None, stepLoss=-0.01, maxSteps=100, wallCoordinates=None, randomWalls=None, chestCoordinates=None, keyCoordinates=None, chestReward=0.1, randomchests=None, randomkeys=None, saveImages=False, killSwitchSteps=6):
        ...
        self.killSwitchCount = 0 # TODO: Implement this
    ...
    def step(self, action):
        ...
        if len(self.killSwitchCoordinates) < 2:
            self.killSwitchCoordinates = np.append(self.killSwitchCoordinates, [self._agent_location], axis=0)
        elif np.any([np.array_equal(self._agent_location, loc) for loc in self.killSwitchCoordinates]):
            ...
            self.killSwitchCoordinates = np.append(self.killSwitchCoordinates, [self._agent_location], axis=0)
        terminated = self.killSwitchCount >= self.killSwitchSteps
        ...
        if np.any([np.array_equal(self._agent_location, loc) for loc in self.chestCoordinates]):
            ...
            if self.chestCoordinates.size == 0:
                terminated = True
