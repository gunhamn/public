...
class Doodler:
    ...
    def save_image(self, image_array, file_name, directory='images'):
        if not os.path.exists(directory):
            os.makedirs(directory)

        # Ensure the image is a numpy array and has the correct shape
        if not isinstance(image_array, np.ndarray) or len(image_array.shape) != 2:
            raise ValueError("Image data is not in expected 2D array format")

        plt.imshow(image_array, cmap='gray')
        plt.axis('off')
        plt.savefig(f"{directory}/{file_name}.png", bbox_inches='tight', pad_inches=0)
...
def save_generated_images(doodler, num_images=10):
    for i in range(num_images):
        # Generate a random case
        case = doodler.gen_random_case()

        # Debug: Print the structure of the case
        print(f"Case {i + 1}: {case}")

        # Extract the image data from 'case'
        # Placeholder for the required transformation:
        # Adjust this line according to the actual structure of 'case'
        image_data = case[0]

        image_data = np.array(image_data).reshape(doodler.canvas_rows, doodler.canvas_cols)

        # Check if image_data is in the correct 2D array format
        if isinstance(image_data, np.ndarray) and len(image_data.shape) == 2:
            # Save the image
            file_name = f"generated_image_{i + 1}"
            doodler.save_image(image_data, file_name)
            print(f"Image {i + 1} saved as {file_name}.png")
        else:
            print(f"Error: Image data for image {i + 1} is not in the correct format.")

def main():
    # Initialize Doodler
    doodler = Doodler(rows=50, cols=50)

    # Save 10 images
    save_generated_images(doodler, 10)
